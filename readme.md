...
**Live preflight probes (`scripts/check_onyx_live_preflight.py --probe`):** **`--probe-mode auth`** issues a cheap **`GET /v1/models`** (same Bearer auth as the expert) to verify the key and route without running a full draft. **`--probe-mode draft`** (default) runs one **`draft_program`** for a single benchmark task — it exercises the full inference path and may **timeout** on slow local GPUs even when auth is correct; use **`--probe-timeout`** / **`--probe-max-tokens`** / **`--probe-task-id`** to tune that check without changing global benchmark defaults.
