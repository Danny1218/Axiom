...
**Smoke check (Phase 81):** **`axiom copilot-doctor`** sends one draft with **`temperature: 0`** (default goal: **`y = x * 2.0`**; override with **`--goal`**). It prints connection status, raw response size, an **`ax_source`** preview, **`parse` / `ir` / `block`** from **`validate_program`**, **`anti_pattern`** lines when **`forbidden_tokens_detected`** (**`assign_colon_eq`**, **`print_call`**) or **`indexed_variable_warning`** / **`output_call_warning`** / **`suspicious_numeric_literal_warning`** appear in expert metadata, and **`neural: yes/no`** from the extracted program. **Exit 0** only when the chat call succeeds and the program compiles to an **`InterpretedBlock`**. Optional **`--timeout`** sets the HTTP timeout (default **120** seconds).
